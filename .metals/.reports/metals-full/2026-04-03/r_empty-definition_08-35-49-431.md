error id: file://<WORKSPACE>/src/main/scala/extprg/pypgx/pypgx.scala:makeRDD.
file://<WORKSPACE>/src/main/scala/extprg/pypgx/pypgx.scala
empty definition using pc, found symbol in pc: makeRDD.
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -scala/sys/process/sc/makeRDD.
	 -scala/sys/process/sc/makeRDD#
	 -scala/sys/process/sc/makeRDD().
	 -sc/makeRDD.
	 -sc/makeRDD#
	 -sc/makeRDD().
	 -scala/Predef.sc.makeRDD.
	 -scala/Predef.sc.makeRDD#
	 -scala/Predef.sc.makeRDD().
offset: 5040
...
object PYPGX {
  ...
  def annotateByPypgx(sc: SparkContext, pyPGXArgs: String, execDir: String): Unit = {
    ...
    // Use makeRDD with explicit partition count instead of .repartition() to avoid a shuffle.
    // repartition() triggers a shuffle stage; if an executor is killed (e.g. OOM from pypgx),
    // its shuffle blocks become unreachable → ShuffleBlockFetcherIterator errors.
    val p = sc.makeRDD@@(indexedSampleFiles, numberOfPartitions)
    ...
    val taskOutputBase = s"${localOutputDir}"

    println(s"[PyPGX] Executing pypgx on ${batches.size} partition(s); per-task tmp base: $taskOutputBase")
    new File(localOutputDir).mkdirs()

    // Run pypgx on each partition, replacing localOutputDir with a unique per-batch path.
    // The output dir index matches the tmpSample_$batchIdx.txt file index.
    val taskOutputDirs: Array[String] = p.mapPartitionsWithIndex { case (partIdx, iter) =>
      import scala.sys.process._

      iter.flatMap { case (batchIdx, sampleFile) =>
        // Each batch gets its own output dir named by batchIdx, mirroring tmpSample_$batchIdx.txt
        val taskOutDir = s"$taskOutputBase/$batchIdx"
    ...
    new scala.reflect.io.Directory(new File(localTmpDir)).deleteRecursively()
