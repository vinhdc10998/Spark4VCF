error id: file://<WORKSPACE>/src/main/scala/extprg/pypgx/pypgx.scala:scala/Array#length().
...
non-local guesses:
	 -scala/sys/process/samples/length.
	 -scala/sys/process/samples/length#
	 -scala/sys/process/samples/length().
	 -samples/length.
	 -samples/length#
	 -samples/length().
	 -scala/Predef.samples.length.
	 -scala/Predef.samples.length#
	 -scala/Predef.samples.length().
offset: 4138
...
object PYPGX {
  ...
  def annotateByPypgx(sc: SparkContext, pyPGXArgs: String, execDir: String): Unit = {
    ...
    println(s"[PyPGX] Found ${samples.len@@gth} samples.")
