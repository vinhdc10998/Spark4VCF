error id: file://<WORKSPACE>/src/main/scala/extprg/pypgx/pypgx.scala:scala/Long#toInt().
file://<WORKSPACE>/src/main/scala/extprg/pypgx/pypgx.scala
empty definition using pc, found symbol in pc: scala/Long#toInt().
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -scala/sys/process/numberOfSamples/toInt.
	 -scala/sys/process/numberOfSamples/toInt#
	 -scala/sys/process/numberOfSamples/toInt().
	 -numberOfSamples/toInt.
	 -numberOfSamples/toInt#
	 -numberOfSamples/toInt().
	 -scala/Predef.numberOfSamples.toInt.
	 -scala/Predef.numberOfSamples.toInt#
	 -scala/Predef.numberOfSamples.toInt().
offset: 4462
...
object PYPGX {
  ...
  def annotateByPypgx(sc: SparkContext, pyPGXArgs: String, execDir: String): Unit = {
    ...
    var adjustedArgs = if (pyPGXArgs.contains(s"--variants $localVariantsPath")) {
      pyPGXArgs.replace(s"--variants $localVariantsPath", s"--variants ${hdfsVariantsPath.toString}")
    } else {
      pyPGXArgs.replace(s" $localVariantsPath", s" ${hdfsVariantsPath.toString}")
    }

    // Remove any manually provided --samples flag/placeholder to avoid duplicates,
    // as we will append it automatically via xargs later.
    adjustedArgs = adjustedArgs.replaceAll("--samples\\s+\\S+", "").trim

    val annotateCmd = execDir + " " + adjustedArgs

    // --- Get sample list using bcftools query -l (more robust) ---
    ...
    val batches = samples.sliding(numberOfSamples.toI@@nt, numberOfSamples.toInt).toList
    ...
    // --- Execute via xargs to pass the sample file path into the --samples flag ---
    // The RDD 'p' contains the local paths to the sample list files.
    // We wrap the command in xargs so each path is passed to --samples.
    val finalCmd = s"xargs -I {} $annotateCmd --samples {}"
    println(s"[PyPGX] Executing: $finalCmd")
    
    new File(localOutputDir).mkdirs()
    p.pipe(finalCmd).collect()
